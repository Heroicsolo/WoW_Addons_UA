...
do
	...
	local myGitHash = "5713a18" -- The ZIP packager will replace this with the Git hash.
	local releaseString = ""
	--[=[@alpha@
...
do
	local DBMdotRevision = "20220322224425" -- The changing version of the local client, changes with every new zip using the project-date-integer packager replacement.
	local DBMdotDisplayVersion = "9.2.8" -- "N.N.N" for a release and "N.N.N alpha" for the alpha duration.
	local DBMdotReleaseRevision = "20220322000000" -- Hardcoded time, manually changed every release, they use it to track the highest release version, a new DBM release is the only time it will change.
